...
:::message
#### いつDIを使うべきか？
「DIPとOCPとデザインパターン」の節でも述べますが、再利用したいモジュールが上位（依存する側）にあり、変更可能性があるモジュールが下位にある場合はOCPの観点からもDIPは必須となります。

デメリット
- DIコンテナなしではオブジェクトの初期化が複雑

なので、すべての依存関係に適用するのではなくコンポーネントやレイヤーレベルのときは行うようにしたほうがいいかも。特にレイヤー感では情報隠蔽の手段としてのDIPのためのDIも考えられる。
